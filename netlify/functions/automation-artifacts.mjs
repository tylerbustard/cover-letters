import { getArtifact, handleHttpError, requireSession } from './_ai-utils.mjs'
...
export const handler = async (event) => {
  try {
    ...
    const artifactId =
      ...
      decodeURIComponent(rawTarget.match(/\/automation\/artifacts\/([^/?\s]+)(?:[/?\s]|$)/)?.[1] || '')
    if (!artifactId) {
    ...
    return buildBinaryResponse(200, artifact.body, {
      'Content-Type': artifact.contentType || 'application/octet-stream',
