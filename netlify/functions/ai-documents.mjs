import { getAiDocumentSnapshot, handleHttpError, requireAiScopes } from './_ai-utils.mjs'
...
const resolvePathMatch = (event) => {
  ...
  return rawTarget.match(
    /\/ai\/documents\/(resume|cover-letter|email-signature)\/([^/?\s]+)(?:[/?\s]|$)/,
  )
}
...
export const handler = async (event) => {
  try {
    ...
    const auth = requireAiScopes(event, ['documents:read'])
    if (auth.response) {
